...
KEY_DP_FAB_SHOP = "Shop Fabrication"
...
class Bolt:
    def __init__(self, grade=None, diameter=None, bolt_type="", bolt_hole_type="Standard",
        ...
        if grade is not None:
            try:
                # Handle single numeric or string input
                if isinstance(grade, (float, np.float64, int, np.integer, str)):
                    self.bolt_grade = [float(grade)]
                # Handle numpy array
                elif isinstance(grade, np.ndarray):
                    self.bolt_grade = grade.astype(float).tolist()
                # Handle iterables (list, tuple, etc.)
                else:
                    self.bolt_grade = [float(g) for g in grade]
            except (ValueError, TypeError) as e:
                logging.error(f"Invalid bolt grade input: {grade}, error: {e}")
                self.bolt_grade = [8.8]  # Default to 8.8 if conversion fails
            self.bolt_grade.sort(key=float)
        if diameter is not None:
            try:
                # Handle single numeric or string input
                if isinstance(diameter, (float, np.float64, int, np.integer, str)):
                    self.bolt_diameter = [float(diameter)]
                # Handle numpy array
                elif isinstance(diameter, np.ndarray):
                    self.bolt_diameter = diameter.astype(float).tolist()
                # Handle iterables
                else:
                    self.bolt_diameter = [float(d) for d in diameter]
            except (ValueError, TypeError) as e:
                logging.error(f"Invalid bolt diameter input: {diameter}, error: {e}")
                self.bolt_diameter = [20.0]  # Default to 20 if conversion fails
            self.bolt_diameter.sort(key=float)
        self.bolt_type = bolt_type
        self.bolt_hole_type = bolt_hole_type
        self.edge_type = edge_type
        try:
            self.mu_f = float(mu_f) if mu_f is not None else 0.3
        except (ValueError, TypeError) as e:
            logging.error(f"Invalid mu_f input: {mu_f}, error: {e}")
            self.mu_f = 0.3  # Default to 0.3 if conversion fails
    ...
    # Rest of the Bolt class and other classes remain unchanged
...
class Weld:
    ...
    def __init__(self, material_g_o="", type=KEY_DP_WELD_TYPE, fabrication= KEY_DP_FAB_SHOP):
        self.design_status = True
        self.type = type
