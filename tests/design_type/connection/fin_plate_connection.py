...
def set_input_values(self, design_dictionary):
    self.mainmodule = "Fin Plate Connection"
    ...
    if self.connectivity in VALUES_CONN_1:
        ...
        self.supporting_section = Beam(designation=design_dictionary[KEY_SUPTNGSEC], 
                                      material_grade=design_dictionary[KEY_SUPTNGSEC_MATERIAL])
    ...
    # Handle plate thicknesses
    plate_thicknesses = design_dictionary.get(KEY_PLATETHK, [])
    if isinstance(plate_thicknesses, str):
        plate_thicknesses = eval(plate_thicknesses)  # Convert string to list if needed
    plate_thicknesses = [float(t) for t in plate_thicknesses]  # Ensure floats
    selected_thickness = float(design_dictionary.get('Plate.Selected_Thickness', 0.0))
    
    self.plate = Plate(thickness=plate_thicknesses, material_grade=design_dictionary[KEY_MATERIAL])
    self.plate.thickness_provided = selected_thickness if selected_thickness in plate_thicknesses else plate_thicknesses[0] if plate_thicknesses else 0.0
...
def member_capacity(self):
    ...
    if self.connectivity in VALUES_CONN_1:
        if self.supported_section.type == "Rolled":
            ...
            self.supported_section.web_height = self.supported_section.depth - (
                        2 * self.supported_section.flange_thickness)
    else:
    ...
    self.supporting_section.tension_yielding_capacity = IS800_2007.cl_6_2_tension_yielding_strength(
        self.supporting_section.area, self.supporting_section.fy)

    # Handle plate thickness filtering
    if isinstance(self.plate.thickness, list) and self.plate.thickness:
        self.thickness_possible = [i for i in self.plate.thickness if i >= self.supported_section.web_thickness]
    else:
        selected_thickness = self.plate.thickness_provided
        if selected_thickness >= self.supported_section.web_thickness:
            self.thickness_possible = [selected_thickness]
        else:
            self.thickness_possible = []
            logger.warning("Selected plate thickness is less than supported section web thickness.")
